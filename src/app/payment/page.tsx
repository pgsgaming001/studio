...
function PaymentPageContent() {
  ...
  useEffect(() => {
    ...
    if (parsedCost < MINIMUM_ORDER_AMOUNT) {
        ...
        setOrderDetails({ totalCost: parsedCost }); // Set for display but payment will be blocked
        return;
    }
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      ...
      if (!razorpayOrderResponse.success || !razorpayOrderResponse.orderId || !razorpayOrderResponse.razorpayKeyId) {
        throw new Error(razorpayOrderResponse.error || "Could not create Razorpay order. Ensure keys are set and amount is valid.");
      }
      ...
      const options = {
        ...
        handler: async function (response: RazorpayPaymentDetails) {
          ...
          const finalPayload: OrderFormPayload = {
            ...(orderDetails as OrderFormPayload), // Type assertion here, as we've validated totalCost
  ...
  const isButtonDisabled = isProcessing || 
                         ...
                         !!error; // Disable if there's an active error message
  ...
  if (error && (!orderDetails.totalCost || orderDetails.totalCost < MINIMUM_ORDER_AMOUNT || !orderDetails.fileName)) {
    // This condition specifically handles critical errors where payment cannot proceed
    return (
  ...
  const displayTotalCost = orderDetails.totalCost?.toFixed(2) ?? '0.00';
      ...
      <Card className="w-full max-w-md shadow-xl rounded-xl">
        ...
        <CardContent className="space-y-6 pt-6">
          ...
          <div className="text-center">
            ...
            <p className={`text-4xl font-bold ${orderDetails.totalCost && orderDetails.totalCost < MINIMUM_ORDER_AMOUNT ? 'text-destructive' : 'text-accent'}`}>
              ₹{displayTotalCost}
            </p>
             {orderDetails.totalCost && orderDetails.totalCost < MINIMUM_ORDER_AMOUNT && (
          ...
          {error && (orderDetails.totalCost && orderDetails.totalCost >= MINIMUM_ORDER_AMOUNT) && ( // Display non-critical errors here
            <div className="p-3 bg-destructive/10 border border-destructive/50 rounded-md text-sm text-destructive text-center">
